...
Ein FAUST-Programm beschreibt keinen Klang, sondern einen Signalprozessor, also eine Maschine mit Eingängen und Ausgängen. Das Schlüsselwort \texttt{process} definiert diesen Signalprozessor. Programm~\ref{prog:faust-minimal} zeigt drei minimale Definitionen.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/abb_faust_operatoren}
\caption{Die fünf Kompositionsoperatoren der Faust-Blockdiagramm}
\label{fig:faust-operatoren}
\end{figure}

Abbildung~\ref{fig:faust-operatoren} stellt dieselben Operatoren als Blockdiagramme dar.
...
Programm~\ref{prog:faust-rauschgenerator} zeigt einen Rauschgenerator mit rekursiver Pseudozufallszahlenerzeugung und einem Regler für die Signalamplitude \parencite[S. ~5]{orlarey:hal-02159014}


\begin{program}
	\caption{Rauschgenerator mit Lautstärkeregler in FAUST}
	\label{prog:faust-rauschgenerator}
	\begin{GenericCode}
...
\begin{enumerate}
	...
	Programm~\ref{prog:faust-aequivalenz} zeigt zwei syntaktisch unterschiedliche Programme mit derselben Signalgleichung.
	
	\begin{program}
		\caption{Semantisch äquivalente FAUST-Beschreibungen}
		\label{prog:faust-aequivalenz}
		\begin{GenericCode}
			process = /(2) : @(10);
			process = *(2) : @(7) : /(4) : @(3);
		\end{GenericCode}
...
Programm~\ref{prog:faust-caching} veranschaulicht den Unterschied zwischen einer direkten Berechnung und einer zwischengespeicherten Berechnung.


\begin{program}
	\caption{Zwischenspeicherung eines gemeinsam verwendeten Teilausdrucks}
	\label{prog:faust-caching}
	\begin{CCode}
